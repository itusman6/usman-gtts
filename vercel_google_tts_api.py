...
# Async TTS generator
async def generate_tts_async(path: str, text: str, voice: str):
    communicate = edge_tts.Communicate(text, voice)
    await communicate.save(path)

# Thread-safe wrapper
def generate_tts(text: str, voice: str, path: str):
...
@app.get("/tts")
def tts(text: str, voice: str = "jenny"):
    voice_id = voice.lower()
    if voice_id not in VOICES:
        raise HTTPException(status_code=400, detail="Invalid voice")

    voice_name = VOICES[voice_id]
    audio_hash = make_hash(text, voice_name)
    audio_path = os.path.join(AUDIO_DIR, f"{audio_hash}.mp3")

    # Generate audio if not cached
    if not os.path.exists(audio_path):
        thread = Thread(target=generate_tts, args=(text, voice_name, audio_path))
        thread.start()
        thread.join()
